//This code is studied and modified from Tee Kay. Retrieved from https://www.youtube.com/watch?v=283rmgvFDE0. 
//And we also study the code from the Minim website. Retriveved from http://code.compartmental.net/tools/minim/. 
...
void setup() {
  ...
  //Set up the three dimension values for our Kincet
...
void draw() {
  ...
  for (int i = 0; i < skeletonArray.size(); i++) {
    ...
    if (skeleton.isTracked()) {
      ...
      //If person claps
      if(PointsTouching(leftjoint, rightjoint) && clapcount > 60){   //如果和手停止一下有重新播放把60改大
         clap = true;
...
void playMusic(){  
  ...
  for(int i = 0; i < player.bufferSize() - 1; i++) {   
    ...
    ellipse(width/2-gloabalDis, height/2, r + player.right.get(i)*i*r*2/player.bufferSize(), r + player.right.get(i)*i*r*2/player.bufferSize());
    ellipse(width/2+gloabalDis, height/2, r + player.right.get(i)*i*r*2/player.bufferSize(), r + player.right.get(i)*i*r*2/player.bufferSize());
  ...
  pushMatrix();
  translate(width/2-gloabalDis, height/2); 
